...
monthDays :: Int -> Int -> [String]
monthDays startWeekDay lastDay = 
    aux startWeekDay lastDay 1
  where 
  aux :: Int -> Int -> Int -> [String]
  aux start last currentDay
   | currentDay > last = []
   | currentDay < start = "    " : aux start last (currentDay + 1)
   | otherwise = formatDay currentDay : aux start last (currentDay + 1)
