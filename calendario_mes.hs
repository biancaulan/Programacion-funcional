...
monthTitle month year = month ++ " " ++ (show year)
...
monthDays startWeekDay lastDay = 
    aux startWeekDay lastDay 1 []
  where 
  aux :: Int -> Int -> Int -> [Linea] -> [Linea]
  aux start last currentDay response
   | currentDay > last = []
   | currentDay < start = aux start last (currentDay + 1) ("   " : response) 
   | otherwise = [week currentDay (currentDay + 6) startWeekDay last] ++ aux start last (currentDay + 7) []
  formatDay :: Int -> String
  formatDay day = if day < 10 then "0" ++ show day else show day
  week :: Int -> Int -> Int -> Int -> Linea
  week start end weekDay lastMonthDay = init (foldl (++) "" [formatDay x ++ " " | x<- [start..end], x <= lastMonthDay])
